...
def fillsheet(key1, value1):
    ...
    ws = wb[value1]
    # 计算当前周所在的列号
    col = getNowYearWeek()[1] - 1 - (int(ws.cell(row=20, column=4).value[2:]) - 4)
    # 坏机失效一次因匹配
    ...
    for m in range(4, col + 1):
        ...
        if m == col - 1:
            ws.cell(row=21, column=m).value = fail_sum / (sale_sum + sale[0][0])
            ...
            ws.cell(row=32, column=m).value = (LCD + TP) / (act_sum + act[0][0])
    ...
    for i in range(len(failure)):
        ...
        sum = sum + 1
    # 合并单元格样式设置
    cell_range = ['A20:A23', 'A25:A35', 'B20:C20', 'B21:C21', 'B22:C22', 'B23:C23', 'B25:C25', 'B34:B35', 'P37:R37',
                  'B26:B29', 'B30:B33']
    ...
    for cell in cell_range:
        style_range(ws, cell, border=border, fill=None, font=font, alignment=al)
...
wb = load_workbook(
    r"/mnt/hgfs/Transsion/Repair rate report/repair_rate_report/transition/week23 F311 repair rate(warranty)1.xlsx")
for key, value in countries.items():
    fillsheet(key, value)
wb.save(
    filename=r"/mnt/hgfs/Transsion/Repair rate report/repair_rate_report/transition/week23 F311 repair rate(warranty)1.xlsx")
