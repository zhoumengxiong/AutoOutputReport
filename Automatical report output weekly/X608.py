...
def write_repair_rate(value1):
    ...
    for m in range(4, col + 1):
        ...
        if sale_sum > 0:
            ...
            if act_sum == 0:
                ...
                ws.cell(row=36, column=m).value = ws.cell(
                    row=66, column=m).value
                ws.cell(row=21, column=m).value = fail_sum / sale_sum
    ...
    for area in cells:
        style_range(ws, area, border=border,
                    fill=None, font=font, alignment=al)
...
model = "X608"
countries = {'Nigeria': '尼日利亚', 'Uganda': '乌干达', 'Cote dIvoire': '科特迪瓦', 'Kenya': '肯尼亚',
             'Tanzania': '坦桑尼亚', 'Egypt': '埃及', 'Zambia': '赞比亚', 'Pakistan': '巴基斯坦',
             'Ghana': '加纳', 'Morocco': '摩洛哥'}
config = {
...
path = r"F:\Transsion\Repair rate report\repair_rate_report\transition\week23 X608 repair rate(warranty)1.xlsx"
wb = load_workbook(filename=path)
# 计算当前周所在的列号
col = getNowYearWeek()[1] - 1 - \
    (int(wb.active.cell(row=20, column=4).value[2:]) - 4)
